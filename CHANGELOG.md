...
- **pg_jsonb_object code mode fix** — `pg.jsonb.object({ name: "John", age: 30 })` now correctly returns `{object: {name: "John", age: 30}}`. Previously, code mode wrapped the input incorrectly, producing nested responses like `{object: {pairs: {...}}}` or `{object: {data: {...}}}`. The fix updates OBJECT_WRAP_MAP to use 'data' key (priority 1 in handler) instead of 'pairs' (priority 3)
