...
- **pg_copy_export truncated flag accuracy** — `pg_copy_export` (`copyExport()`) now correctly returns `truncated: true` only when the actual row count equals the limit (indicating more rows may exist). Previously, `truncated: true` was always returned when the default 500-row limit was applied, even for tables with fewer rows (e.g., 3 rows with `limit=500` incorrectly showed `truncated: true`). This provides accurate feedback about whether data was actually cut off
