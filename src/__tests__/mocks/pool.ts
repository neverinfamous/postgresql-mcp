...
export function createMockConnectionPool(): {
    ...
    return {
        execute: vi.fn().mockResolvedValue({ rows: [], rowCount: 0, executionTimeMs: 5 }),
...
export function configureMockPoolQuery(
    ...
    const originalImpl = pool.query.getMockImplementation() as
        ((sql: string) => Promise<{ rows: Record<string, unknown>[]; rowCount: number }>) | undefined;

    const impl = (sql: string): Promise<{ rows: Record<string, unknown>[]; rowCount: number }> => {
