...
class TestAdapter extends DatabaseAdapter {
    ...
    testRegisterResource(server: unknown, resource: ResourceDefinition): void {
        this.registerResource(server as Parameters<typeof this.registerResource>[0], resource);
    }

    testRegisterPrompt(server: unknown, prompt: PromptDefinition): void {
        this.registerPrompt(server as Parameters<typeof this.registerPrompt>[0], prompt);
    }
}
...
describe('DatabaseAdapter', () => {
    ...
    describe('isConnected', () => {
        ...
        it('should return true after connect', async () => {
            await adapter.connect({ type: 'postgresql', host: 'localhost', port: 5432, database: 'test' });
    ...
    describe('validateQuery', () => {
        describe('dangerous pattern detection', () => {
            it('should reject queries with ; DROP pattern', () => {
                expect(() => adapter.validateQuery('SELECT 1; DROP TABLE users', false)).toThrow(
            ...
            it('should reject queries with ; INSERT pattern', () => {
                expect(() => adapter.validateQuery('SELECT 1; INSERT INTO users VALUES (1)', false)).toThrow(
                    'Query contains potentially dangerous patterns'
                );
            });

            it('should reject queries with ; UPDATE pattern', () => {
                expect(() => adapter.validateQuery('SELECT 1; UPDATE users SET name = "x"', false)).toThrow(
                    'Query contains potentially dangerous patterns'
                );
            });
            ...
            it('should accept safe SELECT queries', () => {
                expect(() => adapter.validateQuery('SELECT * FROM users WHERE id = 1', false)).not.toThrow();
            });

            it('should accept safe INSERT queries when not read-only', () => {
                expect(() => adapter.validateQuery('INSERT INTO users (name) VALUES ($1)', false)).not.toThrow();
            });
        });
        ...
        describe('read-only enforcement', () => {
            it('should reject INSERT in read-only mode', () => {
                expect(() => adapter.validateQuery('INSERT INTO users VALUES (1)', true)).toThrow(
                    'Read-only mode: INSERT statements are not allowed'
                );
            });

            it('should reject UPDATE in read-only mode', () => {
                expect(() => adapter.validateQuery('UPDATE users SET name = $1', true)).toThrow(
                    'Read-only mode: UPDATE statements are not allowed'
                );
            });

            it('should reject DELETE in read-only mode', () => {
                expect(() => adapter.validateQuery('DELETE FROM users WHERE id = 1', true)).toThrow(
                    'Read-only mode: DELETE statements are not allowed'
                );
            });
            ...
            it('should reject CREATE in read-only mode', () => {
                expect(() => adapter.validateQuery('CREATE TABLE test (id int)', true)).toThrow(
                    'Read-only mode: CREATE statements are not allowed'
                );
            });

            it('should reject ALTER in read-only mode', () => {
                expect(() => adapter.validateQuery('ALTER TABLE users ADD COLUMN email text', true)).toThrow(
                    'Read-only mode: ALTER statements are not allowed'
                );
            });
            ...
            it('should reject GRANT in read-only mode', () => {
                expect(() => adapter.validateQuery('GRANT SELECT ON users TO reader', true)).toThrow(
                    'Read-only mode: GRANT statements are not allowed'
                );
            });

            it('should reject REVOKE in read-only mode', () => {
                expect(() => adapter.validateQuery('REVOKE SELECT ON users FROM reader', true)).toThrow(
                    'Read-only mode: REVOKE statements are not allowed'
                );
            });
            ...
            it('should allow EXPLAIN in read-only mode', () => {
                expect(() => adapter.validateQuery('EXPLAIN SELECT * FROM users', true)).not.toThrow();
            });
        });

        describe('input validation', () => {
            it('should throw for empty string', () => {
                expect(() => adapter.validateQuery('', false)).toThrow('Query must be a non-empty string');
            });

            it('should throw for non-string input', () => {
                expect(() => adapter.validateQuery(null as unknown as string, false)).toThrow(
                    'Query must be a non-empty string'
                );
            });

            it('should throw for undefined input', () => {
                expect(() => adapter.validateQuery(undefined as unknown as string, false)).toThrow(
                    'Query must be a non-empty string'
                );
            });
        });
    ...
    describe('registerTools', () => {
        it('should register only enabled tools', () => {
            ...
            adapter.registerTools(mockServer as unknown as Parameters<typeof adapter.registerTools>[0], enabledTools);
        ...
        it('should register no tools if none are enabled', () => {
            ...
            adapter.setMockTools([
                { name: 'pg_query', description: 'test', group: 'core', tags: [], inputSchema: {}, handler: vi.fn() }
            ]);

            adapter.registerTools(mockServer as unknown as Parameters<typeof adapter.registerTools>[0], new Set());
    ...
    describe('registerTool', () => {
        ...
        it('should include annotations in metadata', () => {
            ...
            const metadata = mockServer.tool.mock.calls[0]?.[3] as Record<string, unknown>;
        ...
        it('should include icons in metadata when present', () => {
            ...
            const tool: ToolDefinition = {
                ...
                tags: [],
                inputSchema: {},
                icons: [{ src: 'data:image/svg+xml;base64,test', mimeType: 'image/svg+xml' }],
                handler: vi.fn()
            };
            ...
            const metadata = mockServer.tool.mock.calls[0]?.[3] as Record<string, unknown>;
            expect(metadata['icons']).toEqual([{ src: 'data:image/svg+xml;base64,test', mimeType: 'image/svg+xml' }]);
        });
    });
    ...
    describe('registerResources', () => {
        it('should register all resources', () => {
            ...
            const resources: ResourceDefinition[] = [
                { name: 'schema', uri: 'postgres://schema', description: 'Schema info', handler: vi.fn() },
                { name: 'tables', uri: 'postgres://tables', description: 'Table list', handler: vi.fn() }
            ];

            adapter.setMockResources(resources);
            adapter.registerResources(mockServer as unknown as Parameters<typeof adapter.registerResources>[0]);
    ...
    describe('registerResource', () => {
        ...
        it('should use custom mimeType when provided', () => {
            ...
            const options = mockServer.registerResource.mock.calls[0]?.[2] as { mimeType: string };
    ...
    describe('registerPrompts', () => {
        it('should register all prompts', () => {
            ...
            adapter.registerPrompts(mockServer as unknown as Parameters<typeof adapter.registerPrompts>[0]);
    ...
    describe('registerPrompt', () => {
        ...
        it('should build Zod schema from prompt arguments', () => {
            ...
            const zodShape = mockServer.prompt.mock.calls[0]?.[2] as Record<string, unknown>;
        ...
        it('should invoke prompt handler and return result as message', async () => {
            ...
            const mockHandler = vi.fn().mockResolvedValue({ response: 'test result' });
            const prompt: PromptDefinition = {
                name: 'invoke_prompt',
            ...
            const registeredHandler = mockServer.prompt.mock.calls[0]?.[3] as (args: Record<string, string>) => Promise<unknown>;
    ...
    describe('handler invocation', () => {
        it('should invoke tool handler and JSON stringify object results', async () => {
            ...
            const mockHandler = vi.fn().mockResolvedValue({ data: 'test', count: 42 });
            const tool: ToolDefinition = {
            ...
            const registeredHandler = mockServer.tool.mock.calls[0]?.[4] as (params: unknown) => Promise<unknown>;
            const result = await registeredHandler({});
        ...
        it('should invoke resource handler and JSON stringify object results', async () => {
            ...
            const mockHandler = vi.fn().mockResolvedValue({ tables: ['users', 'orders'] });
            const resource: ResourceDefinition = {
            ...
            const registeredHandler = mockServer.registerResource.mock.calls[0]?.[3] as (uri: URL) => Promise<unknown>;
