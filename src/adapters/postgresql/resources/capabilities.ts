    ...
    return {
        ...
        handler: async (_uri: string, _context: RequestContext) => {
            ...
            const toolCategories: Record<string, ToolCategory> = {
                'Core': { count: 13, description: 'CRUD, schema, tables, indexes, health analysis', examples: ['pg_query', 'pg_insert', 'pg_create_table'] },
                'Transactions': { count: 7, description: 'BEGIN, COMMIT, ROLLBACK, savepoints', examples: ['pg_begin', 'pg_commit', 'pg_savepoint'] },
                'JSONB': { count: 19, description: 'jsonb_set, jsonb_extract, path queries, merge, diff', examples: ['pg_jsonb_get', 'pg_jsonb_set', 'pg_jsonb_merge'] },
                'Text': { count: 11, description: 'Full-text search, trigram, fuzzy matching', examples: ['pg_fts_search', 'pg_trigram_search', 'pg_fuzzy_match'] },
                'Stats': { count: 8, description: 'Descriptive stats, percentiles, correlation, regression', examples: ['pg_stats_summary', 'pg_percentile', 'pg_correlation'] },
                'Performance': { count: 16, description: 'EXPLAIN ANALYZE, plan compare, baseline', examples: ['pg_explain_analyze', 'pg_plan_compare', 'pg_query_baseline'] },
                'Admin': { count: 10, description: 'VACUUM, ANALYZE, REINDEX, configuration', examples: ['pg_vacuum', 'pg_analyze', 'pg_reindex'] },
                'Monitoring': { count: 11, description: 'Database sizes, connections, replication', examples: ['pg_database_size', 'pg_active_connections', 'pg_replication_status'] },
                'Backup': { count: 9, description: 'pg_dump, COPY, physical backup, restore validation', examples: ['pg_dump_schema', 'pg_copy_export', 'pg_restore_command'] },
                'Schema': { count: 10, description: 'Schemas, sequences, views, functions, triggers', examples: ['pg_create_schema', 'pg_create_view', 'pg_list_functions'] },
                'Vector': { count: 14, description: 'pgvector - similarity search, clustering', examples: ['pg_vector_search', 'pg_vector_add_column', 'pg_vector_create_index'] },
                'PostGIS': { count: 12, description: 'Geospatial operations, spatial indexes', examples: ['pg_distance', 'pg_intersection', 'pg_spatial_index'] },
                'Partitioning': { count: 6, description: 'Range/list/hash partitioning management', examples: ['pg_create_partitioned_table', 'pg_create_partition', 'pg_attach_partition'] }
            };
