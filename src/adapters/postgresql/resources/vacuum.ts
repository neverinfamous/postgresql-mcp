...
export function createVacuumResource(adapter: PostgresAdapter): ResourceDefinition {
    return {
        uri: 'postgres://vacuum',
        name: 'Vacuum Status',
        ...
        handler: async (_uri: string, _context: RequestContext) => {
            ...
            `);
            const wraparoundRow = (wraparoundResult.rows?.[0] ?? null) as WraparoundInfo | null;
            const wraparoundInfo = wraparoundRow ?? { datname: '', xid_age: 0, xids_until_wraparound: 0, percent_toward_wraparound: 0 };
            ...
            for (const table of vacuumStats.slice(0, 5)) {
                ...
                if (deadPct > 20) {
                    warnings.push({
                        ...
                        recommendation: 'Run VACUUM ANALYZE ' + table.schemaname + '.' + table.relname + ';'
                    });
                }
            }
