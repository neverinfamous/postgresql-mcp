...
describe("FiniteNumberArray", () => {
    ...
    it("should accept large finite numbers", () => {
        const result = FiniteNumberArray.safeParse([Number.MAX_VALUE, Number.MIN_VALUE]);
        expect(result.success).toBe(true);
    });
});
...
describe("BufferSchema", () => {
    it("should require positive distance", () => {
        ...
        ).toThrow("distance (or radius/meters alias) is required and must be positive");
    });
...
describe("GeocodeSchema", () => {
    ...
    it("should require lat", () => {
        expect(() =>
            GeocodeSchema.parse({ lng: -74 }),
        ).toThrow("lat (or latitude alias) is required");
    });

    it("should require lng", () => {
        expect(() =>
            GeocodeSchema.parse({ lat: 40.7 }),
        ).toThrow("lng (or lon/longitude alias) is required");
    });

    it("should validate lat bounds", () => {
        expect(() =>
            GeocodeSchema.parse({ lat: 95, lng: 0 }),
        ).toThrow("lat must be between -90 and 90");
    });

    it("should validate lng bounds", () => {
        expect(() =>
            GeocodeSchema.parse({ lat: 0, lng: 200 }),
...
describe("CreateViewSchema", () => {
    ...
    it("should require query", () => {
        expect(() =>
            CreateViewSchema.parse({ name: "my_view" }),
