...
export const KcacheResourceAnalysisSchema = z.preprocess(
    ...
    z.object({
        ...
        limit: z.number().optional().describe('Maximum number of queries to return (default: 50)')
    })
);
 ...
 */
export const CitextConvertColumnSchema = CitextConvertColumnSchemaBase.transform((data) => ({
    table: data.table,
 ...
 * Resolves aliases and validates required fields.
 */
export const CitextSchemaAdvisorSchema = z.preprocess(
...
export const LtreeQuerySchemaBase = z.object({
    ...
    type: z.enum(['ancestors', 'descendants', 'exact']).optional()
...
export const LtreeSubpathSchemaBase = z.object({
    ...
    offset: z.number().optional().describe('Starting position (0-indexed, negative counts from end)'),
...
export const LtreeSubpathSchema = z.preprocess((input) => {
    ...
    path: z.string().describe('ltree path (e.g., "Top.Science.Astronomy.Stars")'),
    offset: z.number().describe('Starting position (0-indexed, negative counts from end). Default: 0'),
    length: z.number().optional().describe('Number of labels (omit for rest of path). Alias: len')
}));
...
export const LtreeMatchSchema = z.preprocess((input) => {
    ...
    if (result['pattern'] === undefined) {
        ...
        else if (result['lquery'] !== undefined) result['pattern'] = result['lquery'];
    }
...
export const PgcryptoDecryptSchemaBase = z.object({
    encryptedData: z.string().optional().describe('Encrypted data (base64 from encrypt)'),
