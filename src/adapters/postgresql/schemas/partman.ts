...
function preprocessPartmanParams(input: unknown): unknown {
    ...
    if (result.keepTable !== undefined && result.retentionKeepTable === undefined) {
 ...
 */
export const PartmanCreateParentSchema = z.preprocess(
    preprocessPartmanParams,
    z.object({
        parentTable: z.string().optional().describe('Parent table name (schema.table format). Required.'),
        controlColumn: z.string().optional().describe('Column used for partitioning (timestamp or integer). Alias: control. Required.'),
        interval: z.string().optional()
            ...
            .describe('Partition interval using PostgreSQL syntax (e.g., "1 month", "1 day", "1 week", "10000" for integer). Required.'),
 ...
 */
export const PartmanRunMaintenanceSchema = z.preprocess(
    preprocessPartmanParams,
    z.object({
 ...
 */
export const PartmanShowPartitionsSchema = z.preprocess(
    preprocessPartmanParams,
    z.object({
        parentTable: z.string().optional().describe('Parent table name (schema.table format). Required - specify table to list partitions.'),
 ...
 */
export const PartmanCheckDefaultSchema = z.preprocess(
    preprocessPartmanParams,
    z.object({
        parentTable: z.string().optional().describe('Parent table name to check. Required - specify table to check default partition.')
    })
).default({});
 ...
 */
export const PartmanPartitionDataSchema = z.preprocess(
    preprocessPartmanParams,
    z.object({
        parentTable: z.string().optional().describe('Parent table name (schema.table format). Required - specify table to partition data.'),
 ...
 */
export const PartmanRetentionSchema = z.preprocess(
    preprocessPartmanParams,
    z.object({
        parentTable: z.string().optional().describe('Parent table name (schema.table format). Required.'),
        retention: z.string().nullable().optional().describe('Retention period (e.g., "30 days"). Pass null or omit to disable/clear retention.'),
 ...
 */
export const PartmanUndoPartitionSchema = z.preprocess(
    preprocessPartmanParams,
    z.object({
        parentTable: z.string().optional().describe('Parent table to convert back to regular table. Required.'),
        targetTable: z.string().optional().describe('Target table for consolidated data. Must exist before calling. Alias: target. Required.'),
