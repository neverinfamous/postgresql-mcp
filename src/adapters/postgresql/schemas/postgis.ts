 ...
 * - x/y -> lng/lat
 */
export function preprocessPoint(point: unknown): { lat: number; lng: number } | undefined {
    ...
    // Resolve lng aliases
    const lng = (p['lng'] ?? p['lon'] ?? p['longitude'] ?? p['x']) as number | undefined;
...
export const GeometryColumnSchemaBase = z.object({
    ...
    ifNotExists: z.boolean().optional().describe('Skip if column already exists (returns { alreadyExists: true })')
});

export const GeometryColumnSchema = GeometryColumnSchemaBase.transform((data) => ({
...
export const GeometryDistanceSchemaBase = z.object({
    ...
    maxDistance: z.number().optional().describe('Max distance (in meters by default)'),
    radius: z.number().optional().describe('Alias for maxDistance'),
    distance: z.number().optional().describe('Alias for maxDistance'),
    unit: z.enum(['meters', 'm', 'kilometers', 'km', 'miles', 'mi']).optional().describe('Distance unit (default: meters)'),
    schema: z.string().optional().describe('Schema name (default: public)')
});

export const GeometryDistanceSchema = z.preprocess(
    ...
    return {
        ...
        limit: data.limit,
        maxDistance: rawDistance !== undefined ? convertToMeters(rawDistance, data.unit) : undefined,
        unit: data.unit,
        schema: data.schema
    };
}).refine((data) => data.table !== '', {
...
export const PointInPolygonSchema = z.preprocess(
    preprocessPostgisParams,
...
export const SpatialIndexSchemaBase = z.object({
    ...
    ifNotExists: z.boolean().optional().describe('Skip if index already exists (returns { alreadyExists: true })'),
    schema: z.string().optional().describe('Schema name (default: public)')
});
...
export const BufferSchemaBase = z.object({
    ...
    geom: z.string().optional().describe('Alias for column'),
    geometryColumn: z.string().optional().describe('Alias for column'),
    distance: z.number().optional().describe('Buffer distance (in meters by default)'),
...
export const IntersectionSchemaBase = z.object({
    ...
    geom: z.string().optional().describe('Alias for column'),
    geometryColumn: z.string().optional().describe('Alias for column'),
    geometry: z.string().optional().describe('GeoJSON or WKT geometry to check intersection (e.g., "POINT(0 0)" or GeoJSON)'),
    srid: z.number().optional().describe('SRID for input geometry (auto-detected from column if not provided)'),
...
export const BoundingBoxSchema = z.preprocess(
    preprocessPostgisParams,
...
export const GeocodeSchemaBase = z.object({
    ...
    srid: z.number().int().positive().optional().describe('Spatial Reference ID for output geometry (default: 4326)')
});
...
function preprocessGeocodeParams(input: unknown): unknown {
    ...
    if ((result['lon'] !== undefined || result['longitude'] !== undefined) && result['lng'] === undefined) {
...
export const GeocodeSchema = z.preprocess(
    preprocessGeocodeParams,
    GeocodeSchemaBase
).transform((data) => ({
...
export const GeoTransformSchema = z.preprocess(
    preprocessPostgisParams,
...
export const GeoClusterSchemaBase = z.object({
    ...
    algorithm: z.enum(['dbscan', 'kmeans']).optional().describe('Alias for method'),
    ...
    params: z.object({
        ...
        k: z.number().optional()
    }).optional().describe('Algorithm parameters object (top-level params take precedence)'),
    where: z.string().optional().describe('WHERE clause filter'),
    limit: z.number().optional()
});

export const GeoClusterSchema = z.preprocess(
    preprocessPostgisParams,
    ...
    return {
        ...
        numClusters: data.numClusters ?? data.k ?? data.clusters ?? paramsObj.numClusters ?? paramsObj.k,
        where: data.where,
        limit: data.limit
    };
}).refine((data) => data.table !== '', {
...
export const GeometryBufferSchemaBase = z.object({
    ...
    geojson: z.string().optional().describe('Alias for geometry (GeoJSON format)'),
    distance: z.number().optional().describe('Buffer distance (in meters by default)'),
    radius: z.number().optional().describe('Alias for distance'),
    meters: z.number().optional().describe('Alias for distance'),
    unit: z.enum(['meters', 'm', 'kilometers', 'km', 'miles', 'mi']).optional().describe('Distance unit (default: meters)'),
...
export const GeometryTransformSchemaBase = z.object({
    ...
    fromSrid: z.number().optional().describe('Source SRID (e.g., 4326 for WGS84)'),
    sourceSrid: z.number().optional().describe('Alias for fromSrid'),
    toSrid: z.number().optional().describe('Target SRID (e.g., 3857 for Web Mercator)'),
    targetSrid: z.number().optional().describe('Alias for toSrid')
});
