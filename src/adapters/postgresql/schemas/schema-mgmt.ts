...
const CreateSequenceSchemaBase = z.object({
    ...
    cache: z.number().optional().describe('Number of sequence values to pre-allocate (default: 1)'),
    cycle: z.boolean().optional().describe('Cycle when limit reached (default: no cycle)'),
    ownedBy: z.string().optional().describe('Column that owns this sequence (format: table.column or schema.table.column)'),
    ifNotExists: z.boolean().optional().describe('Use IF NOT EXISTS to avoid error if sequence already exists')
});
...
function preprocessCreateSequenceParams(input: unknown): unknown {
    ...
    if (typeof nameVal === 'string' && nameVal.includes('.') && result['schema'] === undefined) {
