 ...
 */
export const FiniteNumberArray = z.array(z.number()).superRefine((arr: number[], ctx) => {
    const invalidIndexes: number[] = arr
        .map((n: number, i: number) => Number.isFinite(n) ? -1 : i)
        .filter((i: number) => i >= 0);

    if (invalidIndexes.length > 0) {
        const invalidValues = invalidIndexes.map((i: number) => String(arr[i])).join(', ');
        ctx.addIssue({
            code: 'custom',
...
const VectorSearchSchemaBase = z.object({
    ...
    excludeNull: z.boolean().optional().describe('Exclude rows with NULL vectors (default: false)')
});
...
const VectorCreateIndexSchemaBase = z.object({
    ...
    metric: z.enum(['l2', 'cosine', 'inner_product']).optional().describe('Distance metric (default: l2)'),
...
// Transformed schema with alias resolution
export const VectorCreateIndexSchema = VectorCreateIndexSchemaBase.transform((data) => ({
    table: data.table ?? data.tableName ?? '',
    column: data.column ?? data.col ?? '',
    type: data.type,
