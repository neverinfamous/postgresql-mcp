...
describe('pg_list_partitions', () => {
    ...
    it('should list partitions of a table', async () => {
        ...
        mockAdapter.executeQuery.mockResolvedValueOnce({ rows: [{ relkind: 'p' }] });
        // Second call: partition listing
        mockAdapter.executeQuery.mockResolvedValueOnce({
            rows: [
                { partition_name: 'events_2023', partition_bounds: "FOR VALUES FROM ('2023-01-01') TO ('2024-01-01')", size_bytes: 104857600 }
            ]
        });
...
describe('pg_create_partitioned_table', () => {
    ...
    it('should escape quotes within string defaults', async () => {
        ...
        await tool.handler({
            name: 'data',
            columns: [
                { name: 'desc', type: 'text', default: "it's working" }
            ],
            partitionBy: 'hash',
            partitionKey: 'id'
        }, mockContext);
...
describe('pg_create_partition', () => {
    ...
    it('should require subpartitionKey when subpartitionBy is set', async () => {
        ...
        }, mockContext)).rejects.toThrow('subpartitionKey is required');
    });
...
describe('pg_partition_info', () => {
    ...
    it('should get partition info', async () => {
        ...
        mockAdapter.executeQuery.mockResolvedValueOnce({
            rows: [
                { partition_name: 'events_2021', bounds: "FOR VALUES FROM ('2021-01-01') TO ('2022-01-01')", size_bytes: 52428800, approx_rows: 100000 },
                { partition_name: 'events_2022', bounds: "FOR VALUES FROM ('2022-01-01') TO ('2023-01-01')", size_bytes: 78643200, approx_rows: 150000 }
            ]
        });
    ...
    it('should normalize approx_rows -1 to 0 for empty partitions', async () => {
        ...
        // Second call: partition info
        mockAdapter.executeQuery.mockResolvedValueOnce({
            rows: [{ table_name: 'events', partition_strategy: 'RANGE', partition_key: 'event_date', partition_count: 1 }]
        });
        // Third call: partition details with 0 approx_rows
        mockAdapter.executeQuery.mockResolvedValueOnce({
            rows: [{ partition_name: 'events_empty', bounds: "FOR VALUES FROM ('2024-01-01') TO ('2025-01-01')", size_bytes: 8192, approx_rows: 0 }]
        });
...
describe('Parameter Smoothing', () => {
    ...
    describe('pg_create_partitioned_table - partitionBy case normalization', () => {
        it('should accept uppercase RANGE and normalize to lowercase', async () => {
            ...
            const result = await tool.handler({
                name: 'events',
                columns: [{ name: 'id', type: 'bigint' }, { name: 'event_date', type: 'date' }],
                partitionBy: 'RANGE',  // Uppercase - should be normalized
                partitionKey: 'event_date'
            }, mockContext) as { success: boolean; partitionBy: string };
        ...
        it('should accept uppercase LIST and normalize to lowercase', async () => {
            ...
            const result = await tool.handler({
                name: 'orders',
                columns: [{ name: 'id', type: 'bigint' }, { name: 'region', type: 'text' }],
    ...
    describe('pg_create_partition - parameter aliasing', () => {
        it('should accept parentTable as alias for parent', async () => {
            ...
            const result = await tool.handler({
                ...
                name: 'events_2024',
                forValues: "FROM ('2024-01-01') TO ('2025-01-01')"
            }, mockContext) as { success: boolean; parent: string };
        ...
        it('should accept partitionName as alias for name', async () => {
            ...
            const result = await tool.handler({
                ...
                forValues: "FROM ('2024-01-01') TO ('2025-01-01')"
            }, mockContext) as { success: boolean; partition: string };
        ...
        it('should accept from/to and build forValues', async () => {
            ...
            const result = await tool.handler({
                ...
                to: '2025-01-01'
            }, mockContext) as { success: boolean; bounds: string };
    ...
    describe('pg_detach_partition - parameter aliasing', () => {
        ...
        it('should accept partitionTable as alias for partition', async () => {
            ...
            }, mockContext) as { success: boolean; detached: string };
