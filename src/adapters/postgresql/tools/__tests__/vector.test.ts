...
describe('Vector Tools', () => {
    ...
    describe('pg_vector_add_column', () => {
        it('should add vector column with dimensions', async () => {
            ...
            const result = await tool!.handler({
                ...
                column: 'embedding',
                dimensions: 1536
            }, mockContext) as { success: boolean; dimensions: number };
    ...
    describe('pg_vector_insert', () => {
        it('should insert vector data', async () => {
            ...
            const result = await tool!.handler({
                ...
                column: 'embedding',
                vector: [0.1, 0.2, 0.3]
            }, mockContext) as { success: boolean; rowsAffected: number };
    ...
    describe('pg_vector_search', () => {
        it('should search using L2 distance by default', async () => {
            ...
            const result = await tool!.handler({
                ...
                column: 'embedding',
                vector: [0.1, 0.2, 0.3]
            }, mockContext) as { results: unknown[]; metric: string };
    ...
    describe('pg_vector_create_index', () => {
        it('should create IVFFlat index', async () => {
            ...
            const result = await tool!.handler({
                ...
                type: 'ivfflat',
                lists: 100
            }, mockContext) as { success: boolean; type: string };
    ...
    describe('pg_vector_distance', () => {
        it('should calculate L2 distance between vectors', async () => {
            ...
            const result = await tool!.handler({
                vector1: [1, 0, 0],
                vector2: [0, 1, 0]
            }, mockContext) as { distance: number; metric: string };
    ...
    describe('pg_vector_normalize', () => {
        it('should normalize vector to unit length (in-memory)', async () => {
            ...
            const result = await tool!.handler({
                vector: [3, 4]
            }, mockContext) as { normalized: number[]; magnitude: number };
    ...
    describe('pg_vector_aggregate', () => {
        it('should calculate average vector', async () => {
            ...
            const result = await tool!.handler({
                table: 'documents',
                column: 'embedding'
            }, mockContext) as { average_vector: string; count: number };
    ...
    describe('pg_vector_cluster', () => {
        it('should perform K-means clustering', async () => {
            ...
            mockAdapter.executeQuery
                ...
                .mockResolvedValueOnce({
                    rows: [
                        { centroid: '[0.1,0.2]', cluster_size: 5, new_centroid: '[0.15,0.25]' },
                        { centroid: '[0.5,0.6]', cluster_size: 5, new_centroid: '[0.55,0.65]' }
                    ]
                });
            ...
            const result = await tool!.handler({
                ...
                column: 'embedding',
                k: 2
            }, mockContext) as { k: number; centroids: unknown[] };
    ...
    describe('pg_vector_index_optimize', () => {
        it('should recommend no index for small tables', async () => {
            ...
            const result = await tool!.handler({
                table: 'documents',
                column: 'embedding'
            }, mockContext) as { recommendations: Array<{ type: string }> };
    ...
    describe('pg_hybrid_search', () => {
        it('should combine vector and text search', async () => {
            ...
            }, mockContext) as { results: unknown[]; vectorWeight: number; textWeight: number };
        ...
        it('should use custom weights', async () => {
            ...
            const result = await tool!.handler({
                ...
                textQuery: 'test',
                vectorWeight: 0.8
            }, mockContext) as { vectorWeight: number; textWeight: number };
    ...
    describe('pg_vector_performance', () => {
        ...
        it('should run benchmark with test vector', async () => {
            ...
            }, mockContext) as { benchmark: unknown };
    ...
    describe('pg_vector_dimension_reduce', () => {
        it('should reduce vector dimensions (in-memory)', async () => {
            ...
            const result = await tool!.handler({
                ...
                targetDimensions: 10
            }, mockContext) as { originalDimensions: number; targetDimensions: number; reduced: number[] };
    ...
    describe('pg_vector_embed', () => {
        it('should generate demo embedding (in-memory)', async () => {
            ...
            const result = await tool!.handler({
                text: 'Hello world'
            }, mockContext) as { embedding: number[]; dimensions: number; warning: string };
        ...
        it('should use custom dimensions', async () => {
            ...
            const result = await tool!.handler({
                text: 'Test',
                dimensions: 768
            }, mockContext) as { dimensions: number; embedding: number[] };
