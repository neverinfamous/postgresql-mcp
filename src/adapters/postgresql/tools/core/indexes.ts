...
export function createGetIndexesTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        name: 'pg_get_indexes',
        description: 'List indexes with usage statistics. When table is omitted, lists all indexes in the database.',
        group: 'core',
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            if (table === undefined || table === '') {
                ...
                const filtered = schema !== undefined
                    ...
                    : indexes;
                return { indexes: filtered, count: filtered.length };
            }
...
export function createCreateIndexTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            const { name, table, schema, columns, unique, type, where, concurrently, ifNotExists } =
            ...
            if (ifNotExists === true) {
                ...
                `;
                const checkResult = await adapter.executeQuery(checkSql, [schemaName, name]);
...
function preprocessDropIndexParams(input: unknown): unknown {
    ...
    if (result['name'] === undefined) {
        ...
        else if (result['indexName'] !== undefined) result['name'] = result['indexName'];
    }
...
const DropIndexSchema = z.preprocess(
    preprocessDropIndexParams,
    z.object({
        name: z.string().optional().describe('Index name (supports schema.name format)'),
        index: z.string().optional().describe('Alias for name'),
        indexName: z.string().optional().describe('Alias for name'),
        schema: z.string().optional().describe('Schema name (default: public)'),
        ifExists: z.boolean().optional().describe('Use IF EXISTS to avoid errors if index does not exist'),
        cascade: z.boolean().optional().describe('Use CASCADE to drop dependent objects'),
        concurrently: z.boolean().optional().describe('Drop concurrently (does not lock table)')
    })
).transform((data) => ({
    ...data,
