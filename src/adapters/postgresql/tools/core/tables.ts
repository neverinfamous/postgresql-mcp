...
export function createListTablesTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        name: 'pg_list_tables',
        description: 'List all tables, views, and materialized views with metadata. Use limit to restrict results.',
        group: 'core',
...
export function createDescribeTableTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        description: 'Get detailed table structure including columns, types, and constraints. For tables/views only, not sequences.',
        group: 'core',
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            if (!validKinds.includes(relkind)) {
                const kindNames: Record<string, string> = {
                    'i': 'index', 'S': 'sequence', 'I': 'partitioned index',
...
export function createCreateTableTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        description: 'Create a new table with specified columns and constraints. Supports composite primary keys and table-level constraints.',
        group: 'core',
        inputSchema: CreateTableSchema,
        annotations: write('Create Table'),
        icons: getToolIcons('core', write('Create Table')),
        handler: async (params: unknown, _context: RequestContext) => {
            const { name, schema, columns, primaryKey, constraints, ifNotExists } = CreateTableSchema.parse(params);

            const schemaPrefix = schema ? `"${schema}".` : '';
            const ifNotExistsClause = ifNotExists ? 'IF NOT EXISTS ' : '';

            // Determine primary key: prefer explicit primaryKey array, else column-level
            const explicitPK = primaryKey && primaryKey.length > 0 ? primaryKey : null;
            ...
            if (explicitPK && explicitPK.length > 0) {
                ...
                const pkColumnNames = pkColumns.map(col => `"${col.name}"`).join(', ');
                columnDefs.push(`PRIMARY KEY (${pkColumnNames})`);
            }
            ...
            if (constraints && constraints.length > 0) {
                for (const constraint of constraints) {
                    if (constraint.type === 'check' && constraint.expression) {
                        const constraintName = constraint.name ? `CONSTRAINT "${constraint.name}" ` : '';
                        columnDefs.push(`${constraintName}CHECK (${constraint.expression})`);
                    } else if (constraint.type === 'unique' && constraint.columns && constraint.columns.length > 0) {
                        const constraintName = constraint.name ? `CONSTRAINT "${constraint.name}" ` : '';
                        const uniqueCols = constraint.columns.map(c => `"${c}"`).join(', ');
            ...
            return {
                ...
                ...(isCompositePK && { compositePrimaryKey: pkColumns.map(c => c.name) })
            };
        }
    };
}
