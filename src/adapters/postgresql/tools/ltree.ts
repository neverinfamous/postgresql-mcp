...
export function getLtreeTools(adapter: PostgresAdapter): ToolDefinition[] {
    return [
        createLtreeExtensionTool(adapter), createLtreeQueryTool(adapter),
        createLtreeSubpathTool(adapter), createLtreeLcaTool(adapter),
        createLtreeMatchTool(adapter), createLtreeListColumnsTool(adapter),
        createLtreeConvertColumnTool(adapter), createLtreeCreateIndexTool(adapter)
    ];
}
...
function createLtreeSubpathTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            return { originalPath: path, offset, length: length ?? 'to end', subpath: row?.['subpath'] as string, originalDepth: row?.['original_depth'] as number };
        }
    };
}
...
function createLtreeLcaTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            return { paths, longestCommonAncestor: lca ?? '', hasCommonAncestor: lca !== null && lca !== '' };
        }
    };
}
...
function createLtreeMatchTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            return { pattern, results: result.rows ?? [], count: result.rows?.length ?? 0 };
        }
    };
}
...
function createLtreeListColumnsTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            if (schema !== undefined) { conditions.push(`table_schema = $1`); queryParams.push(schema); }
