...
describe('pg_explain', () => {
    ...
    it('should explain a query in text format', async () => {
        mockAdapter.executeQuery.mockResolvedValueOnce({
            rows: [
                { 'QUERY PLAN': 'Seq Scan on users  (cost=0.00..10.50 rows=50 width=100)' }
            ]
        });
...
describe('pg_explain_analyze', () => {
    ...
    it('should run EXPLAIN ANALYZE', async () => {
        mockAdapter.executeQuery.mockResolvedValueOnce({
            rows: [{ 'QUERY PLAN': 'Seq Scan on users (cost=0.00..10.50 rows=50) (actual time=0.015..0.020 rows=50 loops=1)' }]
        });
...
describe('pg_index_stats', () => {
    ...
    it('should return index statistics', async () => {
        mockAdapter.executeQuery.mockResolvedValueOnce({
            rows: [{ indexrelname: 'users_pkey', idx_scan: 1000, idx_tup_read: 5000 }]
        });
...
describe('pg_query_plan_compare', () => {
    ...
    it('should recommend query2 when it has lower cost (costDifference > 0)', async () => {
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ 'QUERY PLAN': [{ Plan: { 'Total Cost': 200 } }] }] })
            .mockResolvedValueOnce({ rows: [{ 'QUERY PLAN': [{ Plan: { 'Total Cost': 50 } }] }] });
    ...
    it('should recommend query1 when it has lower cost (costDifference < 0)', async () => {
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ 'QUERY PLAN': [{ Plan: { 'Total Cost': 30 } }] }] })
            .mockResolvedValueOnce({ rows: [{ 'QUERY PLAN': [{ Plan: { 'Total Cost': 100 } }] }] });
    ...
    it('should use analyze option when provided', async () => {
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ 'QUERY PLAN': [{ Plan: { 'Total Cost': 100 }, 'Execution Time': 1.5 }] }] })
            .mockResolvedValueOnce({ rows: [{ 'QUERY PLAN': [{ Plan: { 'Total Cost': 50 }, 'Execution Time': 0.8 }] }] });
...
describe('pg_performance_baseline', () => {
    ...
    it('should create performance baseline', async () => {
        ...
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ heap_hits: 1000, heap_reads: 10, cache_hit_ratio: 99 }] })
            .mockResolvedValueOnce({ rows: [{ total_seq_scans: 100, total_idx_scans: 5000 }] })
...
describe('pg_connection_pool_optimize', () => {
    ...
    it('should optimize connection pool', async () => {
        ...
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ total_connections: 20, active: 5, idle: 10, idle_in_transaction: 2 }] })
            .mockResolvedValueOnce({ rows: [{ max_connections: 100, reserved_connections: 3 }] })
            .mockResolvedValueOnce({ rows: [{ wait_event_type: 'Lock', wait_event: 'tuple', count: 2 }] });
    ...
    it('should warn when connection utilization exceeds 80%', async () => {
        ...
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ total_connections: 85, active: 70, idle: 10, idle_in_transaction: 2, waiting: 3, max_connection_age_seconds: 100, avg_connection_age_seconds: 50 }] })
            .mockResolvedValueOnce({ rows: [{ max_connections: 100, reserved_connections: 3 }] })
        ...
        expect(result.recommendations).toContainEqual(expect.stringContaining('utilization'));
    });

    it('should warn when idle-in-transaction exceeds active', async () => {
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ total_connections: 20, active: 3, idle: 5, idle_in_transaction: 10, waiting: 0, max_connection_age_seconds: 100, avg_connection_age_seconds: 50 }] })
            .mockResolvedValueOnce({ rows: [{ max_connections: 100, reserved_connections: 3 }] })
            .mockResolvedValueOnce({ rows: [] });
        ...
        expect(result.recommendations).toContainEqual(expect.stringContaining('idle-in-transaction'));
    });

    it('should warn when idle/active ratio is too high', async () => {
        // idle (30) > active (5) * 3
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ total_connections: 40, active: 5, idle: 30, idle_in_transaction: 2, waiting: 0, max_connection_age_seconds: 100, avg_connection_age_seconds: 50 }] })
            .mockResolvedValueOnce({ rows: [{ max_connections: 100, reserved_connections: 3 }] })
            .mockResolvedValueOnce({ rows: [] });
        ...
        expect(result.recommendations).toContainEqual(expect.stringContaining('idle to active'));
    });

    it('should warn when connections are long-lived', async () => {
        // max_connection_age > 3600 seconds (1 hour)
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ total_connections: 20, active: 5, idle: 10, idle_in_transaction: 2, waiting: 0, max_connection_age_seconds: 7200, avg_connection_age_seconds: 3600 }] })
            .mockResolvedValueOnce({ rows: [{ max_connections: 100, reserved_connections: 3 }] })
            .mockResolvedValueOnce({ rows: [] });
        ...
        expect(result.recommendations).toContainEqual(expect.stringContaining('Long-lived connections'));
    });

    it('should return healthy message when no issues', async () => {
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ total_connections: 10, active: 5, idle: 4, idle_in_transaction: 0, waiting: 0, max_connection_age_seconds: 100, avg_connection_age_seconds: 50 }] })
            .mockResolvedValueOnce({ rows: [{ max_connections: 100, reserved_connections: 3 }] })
            .mockResolvedValueOnce({ rows: [] });
        ...
        expect(result.recommendations).toContainEqual(expect.stringContaining('healthy'));
    });
});
...
describe('pg_partition_strategy_suggest', () => {
    ...
    it('should suggest partition strategy', async () => {
        // Mock the 3 parallel queries
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ relname: 'orders', n_live_tup: 50000000 }] })
            .mockResolvedValueOnce({ rows: [{ column_name: 'created_at', data_type: 'timestamp', n_distinct: -1 }] })
            .mockResolvedValueOnce({ rows: [{ table_size: '5 GB', size_bytes: 5000000000 }] });
    ...
    it('should recommend partitioning for tables over 1GB', async () => {
        ...
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ relname: 'large_table', n_live_tup: 1000000 }] })
            .mockResolvedValueOnce({ rows: [{ column_name: 'id', data_type: 'integer', n_distinct: -1 }] })
            .mockResolvedValueOnce({ rows: [{ table_size: '2 GB', size_bytes: 2000000000 }] });
    ...
    it('should recommend partitioning for tables over 10 million rows', async () => {
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ relname: 'large_table', n_live_tup: 15000000, n_dead_tup: 100000, seq_scan: 50, idx_scan: 200 }] })
            .mockResolvedValueOnce({ rows: [{ column_name: 'created_at', data_type: 'timestamp', n_distinct: -1 }] })
            .mockResolvedValueOnce({ rows: [{ table_size: '500 MB', size_bytes: 500000000 }] });
    ...
    it('should suggest RANGE partitioning for timestamp columns', async () => {
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ relname: 'events', n_live_tup: 1000000 }] })
            .mockResolvedValueOnce({ rows: [{ column_name: 'created_at', data_type: 'timestamp', n_distinct: -1 }] })
            .mockResolvedValueOnce({ rows: [{ table_size: '100 MB', size_bytes: 100000000 }] });
        ...
        const rangeStrategy = result.suggestions.find(s => s.strategy === 'RANGE' && s.column === 'created_at');
        expect(rangeStrategy).toBeDefined();
    });

    it('should suggest LIST partitioning for low cardinality columns', async () => {
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ relname: 'orders', n_live_tup: 1000000 }] })
            .mockResolvedValueOnce({ rows: [{ column_name: 'status', data_type: 'varchar', n_distinct: 5, null_frac: 0 }] })
            .mockResolvedValueOnce({ rows: [{ table_size: '100 MB', size_bytes: 100000000 }] });
        ...
        const listStrategy = result.suggestions.find(s => s.strategy === 'LIST' && s.column === 'status');
        expect(listStrategy).toBeDefined();
    });

    it('should suggest HASH partitioning for high cardinality integer columns', async () => {
        mockAdapter.executeQuery
            .mockResolvedValueOnce({ rows: [{ relname: 'users', n_live_tup: 1000000 }] })
            .mockResolvedValueOnce({ rows: [{ column_name: 'user_id', data_type: 'bigint', n_distinct: -1, null_frac: 0 }] })
            .mockResolvedValueOnce({ rows: [{ table_size: '100 MB', size_bytes: 100000000 }] });
