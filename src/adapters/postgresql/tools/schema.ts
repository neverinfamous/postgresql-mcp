    ...
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            return {
                ...
                existed,
                note: existed ? undefined : `Schema '${name}' did not exist (ifExists: true)`
            };
        }
    };
}
...
function createCreateSequenceTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            const parts = [`CREATE SEQUENCE ${ifNotExistsClause}${schemaPrefix}${sanitizeIdentifier(name)}`];
            ...
            await adapter.executeQuery(sql);
            return { success: true, sequence: `${schema ?? 'public'}.${name}`, ifNotExists: ifNotExists ?? false };
        }
    };
}
...
function preprocessDropSequenceParams(input: unknown): unknown {
    ...
    if (typeof result['name'] === 'string' && result['name'].includes('.') && result['schema'] === undefined) {
...
function createListViewsTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            const hasMatViews = views.some((v: Record<string, unknown>) => v['type'] === 'materialized_view');
...
const DropViewSchema = z.preprocess(
    ...
    z.object({
        ...
        schema: z.string().optional().describe('Schema name (default: public)'),
        materialized: z.boolean().optional().describe('Whether the view is materialized'),
        ifExists: z.boolean().optional().describe('Use IF EXISTS to avoid errors'),
        cascade: z.boolean().optional().describe('Drop dependent objects')
    })
);
...
function createDropViewTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            await adapter.executeQuery(sql);
            return { success: true, view: `${schemaName}.${name}`, materialized: materialized ?? false };
        }
    };
}
...
function createListFunctionsTool(adapter: PostgresAdapter): ToolDefinition {
    ...
    const ListFunctionsSchema = z.preprocess(
        ...
        z.object({
            ...
            exclude: z.array(z.string()).optional().describe('Array of extension names/schemas to exclude, e.g., ["postgis", "ltree", "pgcrypto"]'),
            language: z.string().optional().describe('Filter by language (e.g., "plpgsql", "sql", "c")'),
            limit: z.number().optional().describe('Max results (default: 500). Increase for databases with many extensions.')
        })
    );
