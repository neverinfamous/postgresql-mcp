...
function createTextRankTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        inputSchema: z.object({
            ...
            columns: z.array(z.string()).optional().describe('Multiple columns to search (alternative to column)'),
            query: z.string(),
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            const parsed = (params as { table: string; column?: string; columns?: string[]; query: string; config?: string; normalization?: number; select?: string[]; limit?: number });
...
function createTrigramSimilarityTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            const { table, column, value, threshold, select, limit } = TrigramSimilaritySchema.parse(params);
...
function createFuzzyMatchTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        inputSchema: z.object({
            ...
            maxDistance: z.number().optional().describe('Max Levenshtein distance (default: 3, use 5+ for longer strings)'),
            select: z.array(z.string()).optional().describe('Columns to return'),
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            const parsed = (params as { table: string; column: string; value: string; method?: string; maxDistance?: number; select?: string[]; limit?: number });
...
function createRegexpMatchTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            const { table, column, pattern, flags, select, limit } = RegexpMatchSchema.parse(params);
...
function createLikeSearchTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        inputSchema: z.object({
            ...
            column: z.string(),
            pattern: z.string(),
            caseSensitive: z.boolean().optional().describe('Use case-sensitive LIKE (default: false, uses ILIKE)'),
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            const parsed = (params as { table: string; column: string; pattern: string; caseSensitive?: boolean; select?: string[]; limit?: number });
...
function createTextHeadlineTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        inputSchema: z.object({
            ...
            query: z.string(),
            config: z.string().optional(),
            options: z.string().optional().describe('Headline options (e.g., "MaxWords=20, MinWords=5"). Note: MinWords must be < MaxWords.'),
            select: z.array(z.string()).optional().describe('Columns to return for row identification (e.g., ["id"])'),
            limit: z.number().optional().describe('Max results')
        }),
        annotations: readOnly('Text Headline'),
        icons: getToolIcons('text', readOnly('Text Headline')),
        handler: async (params: unknown, _context: RequestContext) => {
            const parsed = (params as { table: string; column: string; query: string; config?: string; options?: string; select?: string[]; limit?: number });
            const cfg = parsed.config ?? 'english';
...
function createFtsIndexTool(adapter: PostgresAdapter): ToolDefinition {
    return {
        ...
        inputSchema: z.object({
            ...
            config: z.string().optional(),
            ifNotExists: z.boolean().optional().describe('Skip if index already exists (default: false)')
        }),
        annotations: write('Create FTS Index'),
        icons: getToolIcons('text', write('Create FTS Index')),
        handler: async (params: unknown, _context: RequestContext) => {
            const parsed = (params as { table: string; column: string; name?: string; config?: string; ifNotExists?: boolean });
            const cfg = parsed.config ?? 'english';
            ...
            return { success: true, index: parsed.name ?? defaultIndexName, config: cfg, skipped: parsed.ifNotExists === true };
        }
    };
}
...
function createTextToVectorTool(adapter: PostgresAdapter): ToolDefinition {
    const ToVectorSchema = z.object({
        ...
        config: z.string().optional().describe('Text search configuration (default: english)')
    });
...
function createTextToQueryTool(adapter: PostgresAdapter): ToolDefinition {
    const ToQuerySchema = z.object({
        ...
        config: z.string().optional().describe('Text search configuration (default: english)'),
        mode: z.enum(['plain', 'phrase', 'websearch']).optional().describe('Query parsing mode: plain (default), phrase (proximity), websearch (Google-like)')
    });
