...
describe('Bug Fixes', () => {
    ...
    describe('pg_hybrid_search select parameter', () => {
        it('should respect select parameter to limit columns', async () => {
            ...
            mockAdapter.executeQuery
                .mockResolvedValueOnce({ rows: [{ data_type: 'USER-DEFINED', udt_name: 'vector' }] }) // type check
                .mockResolvedValueOnce({ rows: [{ id: 1, title: 'test', combined_score: 0.9 }] }); // main query
        ...
        it('should exclude vector columns when select is not provided', async () => {
            ...
            mockAdapter.executeQuery
                .mockResolvedValueOnce({ rows: [{ data_type: 'USER-DEFINED', udt_name: 'vector' }] }) // type check
                .mockResolvedValueOnce({ rows: [{ column_name: 'id' }, { column_name: 'content' }] }) // column list
    ...
    describe('pg_vector_cluster clusters alias', () => {
        it('should accept clusters as alias for k', async () => {
            mockAdapter.executeQuery.mockResolvedValue({
                rows: [{ vec: '[0.1,0.2,0.3]' }, { vec: '[0.4,0.5,0.6]' }, { vec: '[0.7,0.8,0.9]' }]
            });
