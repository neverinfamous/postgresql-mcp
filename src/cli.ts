...
import type { DatabaseConfig, OAuthConfig, TransportType } from './types/index.js';
...
program
    ...
    .action(async (options: CliOptions) => {
        // Set log level
        const logLevel = options.logLevel ?? (process.env['LOG_LEVEL'] as typeof options.logLevel);
        if (logLevel) {
        ...
        try {
            ...
            if (oauthConfig?.enabled) {
                logger.info('OAuth 2.0 authentication enabled', { issuer: oauthConfig.issuer });
            }

            // Determine transport type
            const transport = (options.transport ?? process.env['MCP_TRANSPORT'] ?? 'stdio') as TransportType;
...
function buildDatabaseConfig(options: CliOptions): DatabaseConfig {
    ...
    if (options.postgres) {
        ...
        config.host = options.host ?? process.env['PGHOST'] ?? process.env['POSTGRES_HOST'] ?? 'localhost';
        config.port = options.pgPort ?? parseInt(process.env['PGPORT'] ?? process.env['POSTGRES_PORT'] ?? '5432', 10);
        config.username = options.user ?? process.env['PGUSER'] ?? process.env['POSTGRES_USER'] ?? 'postgres';
        config.password = options.password ?? process.env['PGPASSWORD'] ?? process.env['POSTGRES_PASSWORD'] ?? '';
        config.database = options.database ?? process.env['PGDATABASE'] ?? process.env['POSTGRES_DATABASE'] ?? 'postgres';
 ...
 */
async function buildOAuthConfig(options: CliOptions): Promise<OAuthConfig | undefined> {
    // Check if OAuth is enabled
    const oauthEnabled = options.oauthEnabled ??
    ...
    const clockTolerance = options.oauthClockTolerance ??
        (process.env['OAUTH_CLOCK_TOLERANCE'] ? parseInt(process.env['OAUTH_CLOCK_TOLERANCE'], 10) : 60);

    // Auto-discover JWKS URI if not provided
    if (!jwksUri && issuer) {
        try {
            const discovery = new AuthorizationServerDiscovery({ authServerUrl: issuer });
 ...
 */
async function startStdioServer(adapter: PostgresAdapter, toolFilter?: string): Promise<void> {
...
async function startHttpServer(
    ...
    if (oauthConfig?.enabled && oauthConfig.issuer && oauthConfig.jwksUri && oauthConfig.audience) {
