...
- \`listViews({ includeMaterialized?, truncateDefinition?, limit? })\`: \`{views: [{schema, name, type, definition, definitionTruncated?}], count, hasMatViews, truncatedDefinitions?, truncated?, note?}\`. Default \`limit: 50\` (use \`0\` for all). Default \`truncateDefinition: 1000\` chars (use \`0\` for full definitions)
