...
- \`pg_citext_analyze_candidates\`: Default \`limit: 100\` (use \`0\` for all). Returns \`truncated: true\` + \`totalCount\` when results are limited. Scans tables for TEXT/VARCHAR columns matching common patterns (email, username, name, etc.). Optional \`schema\`, \`table\`, \`limit\` filters. Returns \`{candidates, count, totalCount, truncated, summary, highConfidenceCandidates, mediumConfidenceCandidates, recommendation}\`
