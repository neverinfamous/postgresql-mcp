...
- \`pg_citext_analyze_candidates\`: Default \`limit: 100\` (use \`0\` for all). Default \`excludeSystemSchemas: true\` filters out extension schemas (cron, topology, partman, tiger) when no \`schema\`/\`table\` filter specified—use \`excludeSystemSchemas: false\` to include all. Returns \`truncated: true\` + \`totalCount\` when results are limited. Scans tables for TEXT/VARCHAR columns matching common patterns (email, username, name, etc.). Optional \`schema\`, \`table\`, \`limit\`, \`excludeSystemSchemas\` filters. Returns \`{candidates, count, totalCount, truncated, summary, highConfidenceCandidates, mediumConfidenceCandidates, recommendation, excludedSchemas?}\`
