...
export function needsQuoting(name: string): boolean {
    ...
    if (name !== name.toLowerCase() || name.startsWith('_') || name.includes('$')) {
        return true;
    }
 ...
 */
export function generateIndexName(table: string, columns: string | string[], prefix = 'idx'): string {
    const columnPart = Array.isArray(columns) ? columns.join('_') : columns;
    const name = `${prefix}_${table}_${columnPart}`;

    // Truncate if needed
    const truncated = name.length > MAX_IDENTIFIER_LENGTH
